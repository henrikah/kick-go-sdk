...
func Test_GetUsersByIDMissingAccessToken_Error(t *testing.T) {
	...
	// Assert
	if usersData != nil {
		t.Fatal("Expected usersData to be nil")
	}
...
func Test_GetUsersByIDUnAuthorized_Error(t *testing.T) {
	// Arrange
...
func Test_GetUsersByID_Success(t *testing.T) {
	...
	expectedJSON := `{
		"data": [{
			...
			"name": "test-user-1",
			"profile_picture": "https://test-user-1",
			"user_id": 1
		},
		{
	...
	httpClient := &mocks.MockHTTPClient{
		DoFunc: func(req *http.Request) (*http.Response, error) {
			if req.URL.String() != "https://api.kick.com/public/v1/users?id=1&id=2" {
	...
	// Assert
	if usersData == nil {
		t.Fatal("Expected usersData to not be nil")
	}
	...
	if len(usersData.Data) != 2 {
		t.Fatalf("Expected Data to be 2 slices, got %d", len(usersData.Data))
	}

	if usersData.Message != "test-message" {
		t.Fatalf("Expected Message to be %s, got %s", "test-message", usersData.Message)
	}
}
